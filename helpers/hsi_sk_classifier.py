...
# Helper script for spectral_standardization

def _normalize(array, max_val=None, min_val=None, axis=None):
    """Min-max normalize an array to [0, 1]."""
    if min_val is None:
        min_val = np.min(array, axis=axis, keepdims=True) if axis is not None else np.min(array)
    if max_val is None:
        max_val = np.max(array, axis=axis, keepdims=True) if axis is not None else np.max(array)
    return (array - min_val) / (max_val - min_val + 1e-10)


def spectral_standardization(data, wavenum_1=2700, wavenum_2=3100,
                              num_samp=61, background=None, plot_example=False):
    """
    Remove PBS/water background from raw SRS spectra.

    Parameters
    ----------
    data : np.ndarray, shape (n_pixels, n_channels)
        Raw spectra from load_and_process_image.
    wavenum_1 : int
    ...
    background : np.ndarray, shape (n_channels,)
        PBS background spectrum loaded from srs_params_path.
    plot_example : bool
        If True, plot one example spectrum with and without background.

    Returns
    -------
    spectra : np.ndarray, shape (n_pixels, n_channels)
        Background-subtracted, normalized spectra.
    """
    if background is None:
        raise ValueError("background cannot be None. Load from srs_params_path (params_dataset/).")

    ch_start = int((2800 - wavenum_1) / (wavenum_2 - wavenum_1) * num_samp)

    temp_norm  = _normalize(data)
    ...
    if plot_example:
        idx = np.random.randint(0, spectra_standard.shape[0])
        plt.figure()
        plt.plot(temp[idx],              label='Original')
        plt.plot(background_arr[idx],    label='Background estimate')
        plt.plot(spectra_standard[idx],  label='Standardized')
        plt.title('Spectral Standardization Example')
        plt.legend()
        plt.show()

    peak_idx   = np.argmax(np.mean(spectra_standard, axis=0))
    peak_mean  = np.mean(spectra_standard[:, peak_idx])
    peak_std   = np.std(spectra_standard[:, peak_idx])
    spectra_norm = _normalize(spectra_standard, max_val=peak_mean + 3 * peak_std, min_val=0)
